...
//    }
    ...
    df = spark.sql("select category,trim(label) as label1,sum(impression_count) as impression_count, avg(impression_count) as impression_avg, count(impression_count) as count, arr2str(mergeArray(relate_labels))  as relate_labels from toutiaoIncrement3 group by category,label1")
...
//    df = spark.sql("select category, label, impression_count, impression_avg, count, arr2str(relate_labels) as relate_labels from toutiaoIncrement4")
    ...
    df.write.mode("Overwrite").jdbc("jdbc:mysql://bigdata00:3306/study?SelectMode=cursor&zeroDateTimeBehavior=convertToNull&useServerPrepStmts=true&useSSL=false&useUnicode=true&characterEncoding=utf8", "btoutiao_label", properties)
